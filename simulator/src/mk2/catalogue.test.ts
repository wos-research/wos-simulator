...
test('validated catalogue policy restores one original attack cell and preserves every other profile and axis', () => {
  ...
  for (const [id, troop] of Object.entries(prior.troopStats)) {
    ...
    if (id !== 'lancer_t10_fc4') assert.deepEqual(current.troopStats[id], troop);
  }
  assert.deepEqual(changed, ['lancer_t10_fc4.attack']);
