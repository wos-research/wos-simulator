...
**Current reviewed corpus:** [536 reports](mk2-reviewed-research536-20260914.md), with 516 exact matches, 20 unresolved cases and 535 correct observed winner classifications under catalogue 19. Fixtures, recorded seeds, independent timestamps and full anonymized combat reports are included. Private candidate results remain separate. [Catalogue 19’s 516/516 checkpoint](installed-catalogue19-validation-20260914.md) describes the earlier corpus; dated counts below are historical.

The [dashboard/CLI compatibility review](mk2-compatibility-review-20260914.md) records the repaired input representations and remaining limitations.
...
**Research direction (September13):** the user abandoned the proposed rollback and directed continued research from the current Mk2 baseline. Protect established source behavior and prioritize unresolved RNG timing, skill ordering and random-number consumption. The broad FC flooring override exceeded its original evidence scope; isolated comparison results do not authorize extrapolation or prove a coefficient. The user subsequently authorized independently tested, narrowly scoped additions for missing RNG timing/consumption/count behavior. Changing already established original statistics, probabilities or combat behavior still requires specific approval. See [the scope clarification](mk2-rng-scope-20260913.md) and [latest twenty reports](mk2-continuation-20260913.md).
...
Matching a report's recorded seed is diagnostic replay. Independently deriving the game's seed from battle time remains unresolved. The original160 controlled cases carry `timestampSource: "derived-report-seed-minus-one"`; those constructed values are not independent seeding evidence. Later fixtures also include independently linked capture timestamps, retaining the original recorded seed and any known disagreement. Skill-credited wounded conversion, general hero interactions, and untested troop combinations are outside this validation.
