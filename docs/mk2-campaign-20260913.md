...
## Terminal Crystal Lance: cancellation versus retargeting

Ten fresh hero-free battles use 500 T5 FC4 Lancers against 1 T5 FC1 Infantry plus 500 T5 non-FC Marksmen, five each direction. These battles occurred before the characters' subsequent Lancer-building upgrades. Both actual v13 kernels reproduce all ten winners, all 60 survivor values and all ten explicitly encoded Crystal Lance counts, including an explicit zero. No kernel change was needed.

A prospective private contrast changed only where an already-generated Lance extra hit goes when the normal hit kills the last Infantry while Marksmen survive: current cancellation versus transferring that extra hit to the surviving Marksmen. Seeds, chance probabilities, stats and damage coefficients remained fixed. The current model matches 10/10; the retarget alternative matches 9/10. Campaign case143 separates them: the game and current kernel leave 427 Lancers, while retargeting predicts 436. Both predict the reported three Lance activations. The other nine reports, including all five reverse cases, do not distinguish these alternatives and are retained as such. This supports cancellation in the tested context; it does not establish every possible extra-hit targeting rule.

Each direction's first chronological report passed before explicit release of its four remaining cases. Both first reports were nondiscriminating. Independent direct-wire review verified seeds, quantities, all 16 captured modifier fields per actor, unchanged expert context, empty heroes/extra effects and all available chance counts. The original integer-versus-string expert-map key mismatch stopped preparation before replay; an additive normalization fix preserved every key value and array order and rejected canonical-key collisions. No player stat or expert level was adjusted.

Anonymized fixtures140-149 preserve the captured inputs, recorded seeds and observations. The passing corpus is now **314 reports**:160 original,149 campaign and the five existing D1 reports. Two published mixed-skill failures remain separate. The normal CLI and campaign regression pass all149 campaign reports; fixture integrity checks pass all ten additions. The last implementation-wide313-test/TypeScript validation remains applicable because this update changes only fixtures and documentation. Replay version remains `expedition-mk2-lua54-catalogue-13`; independent timestamp-derived seeding, mixed-skill candidates, other skill levels, heroes and T12 remain outside this new evidence.

## FC5 T10 Lancer health: first five reports do not distinguish the values

Five fresh battles use 500 T10 FC5 Lancers attacking 1,500 T5 non-FC Marksmen. Both actual v13 kernels reproduce every winner, all 30 survivor values and all ten explicitly encoded Ambusher/Lance activation counts, including an explicit zero for each skill in one report. The first chronological report gated release of the remaining four; independent direct-wire review verified the original seeds, quantities, all 16 modifiers per actor, unchanged experts, and empty heroes/extra effects.

The prospective alternative changes only T10 FC5 Lancer health from the current 596 to the original catalogue value 597. Both alternatives match all five reports. Lancer survivors are 444, 444, 434, 433 and 446; the first two are distinct fights and remain separate. These reports are compatibility evidence, not evidence choosing either health value. No stat, skill, probability or kernel implementation is changed. Reverse-direction collection is in progress and is not included in this publication checkpoint.

Anonymized campaign fixtures 150-154 preserve the original predictive inputs, observations and recorded seeds. The passing corpus now contains **319 reports**: 160 original, 154 campaign and the five existing D1 reports. The campaign regression and ordinary CLI pass all 154 campaign reports; integrity checks verify the five additions against their independently reviewed capture-derived inputs. Mixed-skill research reports are documented separately in [pending research](mk2-pending-research.md) and are not included in this passing count. Recorded-seed replay still does not independently establish the game's seeding time.


## Reverse FC5 health controls and upgraded second-pair backgrounds

Five reverse health battles, with 1,500 T5 non-FC Marksmen attacking 500 T10 FC5 Lancers, also match both fixed health596/597 alternatives. Both values now reproduce all ten fights. Reverse Lancer survivors are445,449,444,445,444; all ten explicitly supplied reverse Ambusher/Lance counts agree. Independent raw review confirms seeds, all six survivor slots, unchanged16-field modifiers and experts. No health value is selected. These five reports are added as campaign155-159. The next prospective pair increases only the Marksman count to5,000: that was the smallest tested panel pair with at least16/20 arbitrary-seed distinctions in both directions (20/20 observed in each planning panel). It is a forecast, not in-game validation or seed selection; new collection is separate.

Two new account-background battles use500 T5 FC5 Lancers versus500 T5 FC5 Infantry, one each direction. These were captured after the user upgraded both accounts' Lancer buildings. Both actual v13 kernels match both winners, twelve survivor values, and all four explicitly observed activation counts. With Lancers attacking, Infantry survive190 and the report records Lance11/Shield43. With Infantry attacking, Infantry survive193 and the report records Lance10/Shield49. All16 captured modifiers per actor and expert context remain consistent across roles. This pair checks Lance15% plus Shield37.5% under the new profiles and account context; it does not uniquely prove every coefficient or mixed-army ordering rule. No fitting or implementation change was needed.

Campaign160-161 preserve these two new backgrounds. The complete passing collection is now **326 reports**:160 original,161 campaign and five existing D1 reports, with1,956 survivor checks and549 explicitly supplied chance counts. Fifteen shared mixed-skill reports remain known production failures in their separate files. All seven additions preserve inputs, observations and recorded seeds and pass private-identifier integrity checks. The campaign regression and ordinary CLI pass all161 campaign records with zero errors, warnings or failures. No kernel or dashboard source changed.
